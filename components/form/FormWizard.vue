<template>
  <form @submit="onSubmit">
    ...
    <div v-if="!successSubmit">
      ...
      <div class="flex space-x-3">
        ...
        <button
          ...
          class="w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 sm:py-3.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
        >
          {{ isLastStep ? "Отправить письмо" : "Следующее:" }}
          {{ !isLastStep ? steps[currentStepIdx + 1].name : "" }}
        </button>
      </div>
    </div>
    ...
    <div v-else>
      ...
      <h1
        class="flex flex-col gap-4 mb-2 font-semibold tracking-tight text-gray-900 leding-tight dark:text-white"
      >
        <!-- Подтверждено -->
        <span class="text-2xl">Ваш аккаунт зарегестрирован!</span>

        <span class="text-lg">
          Для перехода в профиль вашего учреждения ожидайте подтверждения.
        </span>
      </h1>
